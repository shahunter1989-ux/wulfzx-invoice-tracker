...
function drawHero(page: PDFPage, bold: PDFFont, x: number, top: number, rightX: number, rightWidth: number, theme: PdfTheme) {
  ...
  drawText(page, "WZXU", rightX, top - 24, 18, bold, titleColor);
  ...
  drawText(page, "WZXU", rightX + rightWidth - 83, top - 65, 14, bold, titleColor);
}
...
function drawFooter(page: PDFPage, font: PDFFont, bold: PDFFont, x: number, width: number, theme: PdfTheme) {
  ...
  drawText(page, "WZXU", x + width - 48, 28, 8, bold, theme.primary);
  drawText(page, "invoice by WZXU", x + width - 78, 14, 8, font, MUTED);
}
...
function getPdfTheme(value: string | null | undefined): PdfTheme {
  ...
  if (id === "wulfzx_blueprint") {
    return { id, primary: BLUE, accent: GOLD, heading: "WZXU Blueprint", subheading: "AI COMPANY", footer: "THANK YOU!" };
  }
